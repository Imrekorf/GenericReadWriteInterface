...
private:
	...
	// readability
	template<typename T> using dereftype = typename std::remove_pointer<T>::type;
...
public:
	...
	std::size_t>::type	write(const T  buffer, const std::size_t&& size) 	   { return _write((const char*)buffer, size * sizeof(dereftype<T>)) / sizeof(dereftype<T>); }
	...
	template<typename T> typename std::enable_if<
		std::is_pointer<T>::value && !is_container<T>::value, 
	std::size_t>::type 	read(T buffer, const std::size_t size) 		  { return _read((char*)buffer, size * sizeof(dereftype<T>)) / sizeof(dereftype<T>); }
	 ...
	 * @return std::size_t the amount of T objects written */
	template<typename T, std::size_t size> typename std::enable_if<
		!is_container<T>::value && !is_iterator<T>::value, 
	 ...
	 * @param first iterator pointing to the start of range
	 * @param last  iterator pointing to the end of range
	 * @param p the predicate function to call for every element read from interface. Should accept container element type. Use to mutate read element before writing to container. Should return container's element type
	 * @return InputIt::iterator pointing to the last element send */
	template<typename InputIt> constexpr typename std::enable_if<
		!is_container<itertype<InputIt>>::value && is_iterator<InputIt>::value,
	InputIt>::type 	read(InputIt first, InputIt last, std::function<InputIt(itertype<InputIt> val)> p) {
		itertype<InputIt> iter_buffer;
		for(; first!=last; ++first)
			read(iter_buffer), *first = p(iter_buffer);
		return last;
	}
	/** @brief Writes a container to the interface, executing a predicate for every element read, and passing a reference of the current iterator to the predicate
	 * Useful when reading into a map or special container which needs skips after every read. Predicate modifies value in place.
	 * @tparam InputIt The iterator type
	 * @param first iterator pointing to the start of range
	 * @param last  iterator pointing to the end of range
	 * @param p the predicate function to call for every element read from interface. Should accept container element type reference and iterator reference. Should return void
	 * @return InputIt::iterator pointing to the last element send */
	template<typename InputIt> constexpr typename std::enable_if<
		!is_container<itertype<InputIt>>::value && is_iterator<InputIt>::value,
	InputIt>::type 	read(InputIt first, InputIt last, std::function<void(itertype<InputIt>& val, InputIt& curr)> p) {
		for(; first!=last; ++first)
			read(*first), p(*first, first);
		return last;
	}
